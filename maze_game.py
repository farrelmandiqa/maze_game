...
#horizontal movement first
        ...
        if self.y_speed > 0: # going down
            for p in platforms_touched :
                #we're checking which of the platforms is the highest from the ones below, aligning with it, and then take it as pur support:
...
class Bullet(GameSprite):
    ...
    def update(self):
        self.rect.x += self.speed
        #disapperars after reaching the edge of the screen
    ...
    for e in event.get():
        if e.type == QUIT:
            ...
            if e.key == K_LEFT:
                ...
                packman.y_speed = 0
    if finish != True:
        window.fill(back)#fill the window with color(background,(0, 0)) 
